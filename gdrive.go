...
import (
	...
	"github.com/anybotics/afero-gdrive/iohelper"
	"github.com/anybotics/afero-gdrive/log"
)
...
func (d *GDriver) SetRootDirectory(path string) (*FileInfo, error) {
	rootNode, err := getRootNode(d.srv, d.drive)
	if err != nil {
...
func (d *GDriver) getFileWriter(fi *FileInfo) (io.WriteCloser, chan error, error) {
	...
	go func() {
		...
		_, err := d.srv.Files.Update(fi.file.Id, nil).Fields(fileInfoFields...).Media(reader).SupportsAllDrives(true).Do()
...
func getRootNode(srv *drive.Service, drive string) (*FileInfo, error) {
	root, err := srv.Files.Get(drive).Fields(fileInfoFields...).SupportsAllDrives(true).Do()
	if err != nil {
		return nil, &DriveAPICallError{Err: err}
	}
